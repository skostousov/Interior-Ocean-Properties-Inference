...
class PaperlikeDataset(TorchDataset):
    def __init__(self, transform = None, target_transform = None, downsample = False, normalize=True, filepath=None, grid_size = cfg['data']['grid_size']):
        ...
        self.annotations_map = F.avg_pool2d(torch.tensor(self.annotations_map, dtype=torch.float32), kernel_size=12, stride=12).numpy()
        self.annotations_map = F.interpolate(torch.tensor(self.annotations_map, dtype=torch.float32), size=(self.feature_map.shape[-2], self.feature_map.shape[-1]), mode='nearest').numpy()
        assert self.feature_map.shape[-2] == self.annotations_map.shape[-2], "Feature map and annotation map do not match in latitude dimension"
        assert self.feature_map.shape[-1] == self.annotations_map.shape[-1], "Feature map and annotation map do not match in longitude dimension"
...
if __name__ == "__main__":
    ...
    print(dataset.grid_and_centre_coords_and_temp_unit[200])
    print(dataset[1][1])
