...
class DA_CNN_FULL(nn.Module):
    def __init__(self, in_channels, out_channels=1, grid_size=21, first_layer_filters=64, kernel_size=3, dropout=0.0, dropout2=0.0):
        ...
        self.output_conv = nn.Sequential(
            nn.Conv2d(fuse_conv_filters, 128, kernel_size=3, padding=1),
            nn.ReLU(),
            nn.Conv2d(128, 64, kernel_size=3, padding=1),
            nn.ReLU(),
            nn.Conv2d(64, 32, kernel_size=3, padding=1),
            nn.ReLU(),
            nn.Conv2d(32, out_channels, kernel_size=1)  # Output layer
        )
    def forward(self, x):
...
if __name__ == "__main__":
    import torch
    model = DA_CNN_FULL(6, 1)
    print(model)
    x = torch.randn(2, 6, 100, 200)
    y = model(x)
    print(y.shape)
