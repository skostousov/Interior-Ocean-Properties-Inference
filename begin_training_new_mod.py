...
def main(args):
    ...
    data = TemporalDataset(transform=data_aug, mld_res=mld_res, feature_res=feature_res, season=season)

    batch_size = args.batch_size
    epochs = cfg['training']["epochs"]

    model = models[args.model][0](data[0][0].shape[0], data[0][1].shape[0], grid_size=data.grid_size, **models[args.model][1])
    ...
    for epoch in range(0, num_epochs):
        ...
        if val_loss < best_loss:
            ...
            torch.save(best_checkpoint, save_dir / 'best_checkpoint.pt')
            torch.save(model, save_dir / 'best_model')
            print(f"New best model found at epoch {epoch + 1} with loss {val_loss:.4f}\n\n")
